...
#if defined(HAVE_LINUX_COMPATIBLE_GETRANDOM)
...
/* LCOV_EXCL_START */
    ...
    *(uint32_t *)(void *)&stream.key[crypto_stream_chacha20_KEYBYTES - 4] ^= (uint32_t)r;
#endif
    /* LCOV_EXCL_STOP */
}
...
randombytes_internal_random(void)
{
    ...
    COMPILER_ASSERT(((sizeof stream.rnd32) - (sizeof stream.key)) % sizeof val == (size_t)0U);
